...
forcedGetCQIpRecognition()
{
  CONQSERVER_IP=${FORCE_CONQUEROR_IP}
  echo "Forcing Conqueror server ip to ${CONQSERVER_IP} as indicated from params" | ${TEE}
}

if  [ -z "${FORCE_CONQUEROR_IP}" ]
then
  normalGetCQIpRecognition
else
  forcedGetCQIpRecognition
